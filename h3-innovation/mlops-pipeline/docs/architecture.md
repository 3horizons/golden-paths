...
The pipeline uses Azure ML managed compute clusters that scale to zero between runs. Model versions are tracked in Azure ML Model Registry with performance metrics as tags. Canary deployments route 10% of traffic to the new model for 24 hours before full promotion. Data drift detection runs hourly and triggers retraining if drift exceeds the configured threshold.
