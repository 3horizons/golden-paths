# Architecture — AI Model Evaluation Pipeline
...
The pipeline uses Azure AI Foundry's Evaluation SDK to call the model under test and score responses using AI-assisted metrics (a judge model scores each response). Responsible AI evals use Azure Content Safety to detect harmful outputs. Results are stored in Azure Blob Storage and surfaced in the Grafana dashboard via a JSON datasource.
