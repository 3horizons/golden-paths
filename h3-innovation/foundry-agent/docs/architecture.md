...
The agent runs on Azure AI Agent Service (managed Azure OpenAI + file search + code interpreter). Tools are Python functions decorated with `@agent_tool` and registered automatically at startup. The RAG pipeline uses Azure AI Search with vector + hybrid retrieval. Content Safety screens every user message before sending to the model.
