# Architecture — GitHub Copilot Extension
...
Copilot Extensions communicate with GitHub's API via the `@octokit/core` SDK. The extension server receives server-sent events (SSE) streams and responds with SSE. OAuth ensures only authenticated GitHub users can invoke the extension. Azure integrations use Workload Identity (no static secrets).
