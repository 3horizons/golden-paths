...
The orchestrator routes tasks to specialist agents based on task type and agent availability. Human-in-the-loop gates pause execution at configurable checkpoints, sending an approval request via Teams/Slack webhook. Agent conversation history is persisted in Azure Cosmos DB for fault tolerance and auditability.
