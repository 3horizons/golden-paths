# Architecture — SRE Agent Integration
...
The SRE agent subscribes to Alertmanager webhooks. When an alert fires, the agent: (1) fetches observability data from Prometheus/Loki, (2) sends context to GPT-4o for RCA, (3) proposes remediation actions, (4) executes approved actions (or auto-executes if enabled and confidence > 90%). All actions are logged to an audit trail in Cosmos DB. Human approval gates are configurable per action type.
