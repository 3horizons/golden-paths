# Architecture — API Microservice (Full-Featured)
...
Database credentials are injected at runtime from Kubernetes Secrets (managed by External Secrets Operator from Azure Key Vault). The service uses SQLAlchemy async engine with connection pooling to handle concurrent requests efficiently. Redis caching uses a write-through pattern with configurable TTL per endpoint.
