...
ADF pipelines are defined as ARM JSON and deployed via GitHub Actions using `az datafactory pipeline create`. Airflow DAGs run on the platform-managed Airflow instance in AKS. Data quality checks fail the pipeline if >5% of records are invalid.
