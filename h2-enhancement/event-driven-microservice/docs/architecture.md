...
The outbox pattern ensures events are only published after the database transaction commits. The Redis idempotency store uses a 24-hour TTL — longer than the maximum Service Bus lock duration — to prevent processing the same message twice during retries.
