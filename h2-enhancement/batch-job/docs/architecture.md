...
Failed jobs write to an Azure Service Bus dead-letter topic so no work is silently lost. The `CronJob` uses `concurrencyPolicy: Forbid` to prevent overlapping runs. Prometheus alert fires after 2 consecutive failures to give the on-call team time to investigate.
