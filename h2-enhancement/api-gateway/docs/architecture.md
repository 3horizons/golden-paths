...
APIM is deployed in Internal VNet mode, accessible only from within the platform VNet. External access goes through Azure Application Gateway (WAF). JWT validation uses the Azure AD JWKS endpoint so no secrets are needed in policies.
