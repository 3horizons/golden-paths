# Architecture — Reusable GitHub Actions Workflows
...
Reusable workflows are called via the `workflow_call` trigger. They accept structured inputs and secrets, and return outputs. This pattern centralizes CI/CD logic so improvements (e.g., upgrading the Trivy scanner version) propagate to all callers without touching each repo.
