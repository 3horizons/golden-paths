...
This template applies controls aligned with OWASP Top 10, CIS GitHub Benchmark, and the platform's LGPD/SOC 2 requirements. The `detect-secrets` baseline (`--list-all-plugins`) blocks commits containing credentials before they reach the remote. CodeQL runs on every PR and weekly schedule.
